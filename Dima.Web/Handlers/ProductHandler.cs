...
public class ProductHandler(IHttpClientFactory httpClientFactory) : IProductHandler
{
    ...
    => await _client.GetFromJsonAsync<PagedResponse<List<Product>?>>($"v1/products?Page={request.PageNumber}&PageSize={request.PageSize}")
    ...
    => await _client.GetFromJsonAsync<Response<Product?>>($"v1/products/slug/{request.Slug}") ?? new Response<Product?>(null, 400, "Não foi possível obter o produto");
}
