...
public class OrderHandler(IHttpClientFactory httpClientFactory) : IOrderHandler
{
    ...
    => await _client.GetFromJsonAsync<PagedResponse<List<Order>?>>($"v1/orders?Page={request.PageNumber}&PageSize={request.PageSize}") ?? new PagedResponse<List<Order>?>(null, 400, "Não foi possível obter os pedidos");
    ...
    public async Task<Response<Order?>> RefundAsync(RefoundOrderRequest request)
    {
        ...
        return await result.Content.ReadFromJsonAsync<Response<Order?>>() ?? new Response<Order?>(null, 400, "Não foi possível reembolsar o pedido");
    }
}
